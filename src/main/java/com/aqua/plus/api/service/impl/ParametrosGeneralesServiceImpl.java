...
import com.aqua.plus.api.service.IParametrosGeneralesService;
...
import com.aqua.plus.commons.maps.ParametrosGeneralesMapper;
...
public class ParametrosGeneralesServiceImpl implements IParametrosGeneralesService {
	...
	public ResponseEntity<ResponseDTO> save(ParametrosGeneralesDTO parametrosGeneralesDTO) {
		...
		try {
			boolean isUpdate = parametrosGeneralesDTO.getId() != null
					&& parametrosGeneralesRepository.existsById(parametrosGeneralesDTO.getId());
	...
	/**
	 * Lista Parametros Generales filtrando por codigoPadre.
	 * 
	 * @param codigoPadre (obligatorio) ej: "ESTADOS_NOVEDAD"
	 */
	@Override
	@Transactional(readOnly = true)
	public ResponseEntity<ResponseDTO> findByCodigoPadre(String codigoPadre, Boolean soloActivos) {
		log.info("Listar ParametrosGenerales por codigoPadre={}, soloActivos={}", codigoPadre, soloActivos);

		try {
			if (codigoPadre == null || codigoPadre.isBlank()) {
				return ResponseEntity.badRequest().body(ResponseDTO.builder().success(false)
						.message("codigoPadre es obligatorio").code(HttpStatus.BAD_REQUEST.value()).build());
			}

			List<ParametrosGeneralesEntity> list = Boolean.TRUE.equals(soloActivos)
					? parametrosGeneralesRepository
							.findByCodigoPadreIgnoreCaseAndActivoTrueOrderByDescripcionAsc(codigoPadre)
					: parametrosGeneralesRepository.findByCodigoPadreIgnoreCaseOrderByDescripcionAsc(codigoPadre);

			var dtoList = parametrosGeneralesMapper.listEntityToDtoList(list);

			return ResponseEntity.ok(ResponseDTO.builder().success(true).message(Constantes.CONSULTED_SUCCESSFULLY)
					.code(HttpStatus.OK.value()).totalCount((long) dtoList.size()).response(dtoList).build());

		} catch (Exception e) {
			log.error("Error al listar ParametrosGenerales por codigoPadre", e);
