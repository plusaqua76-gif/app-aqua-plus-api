...
public class TarifaConceptoServiceImpl implements ITarifaConceptoService {
	...
	public ResponseEntity<ResponseDTO> crearTarifaConcepto(Map<String, Object> payload) {
	    ...
	    try {
	        ...
	        if (isBatch) {
	            ...
	            for (int i = 0; i < items.size(); i++) {
	                try {
	                    ...
	                    // ── GET existencia ──────────────────────────────────────────
	                    MapSqlParameterSource paramsGet = new MapSqlParameterSource()
	                            .addValue("idEmpresa",      idEmpresa)
	                            .addValue("idTipoTarifa",   idTipoTarifa)
	                            .addValue("idTipoConcepto", idTipoConcepto);
	                    ...
	                    if (statusGet == 200) {
	                        // ── EXISTE → UPDATE ─────────────────────────────────────
	                        Map<String, Object> resp = objectMapper.convertValue(
	                                mapGet.get("response"), new TypeReference<>() {});
	                        ...
	                        if (item.get("idTipoUso") instanceof Number n) {
	                            jsonUpd.put("idTipoUso", n.intValue());
	                        }
	                        ...
	                        MapSqlParameterSource paramsUpd = new MapSqlParameterSource()
	                                .addValue("jsonData", jsonbParamUpd);
	                        ...
	                        if (codeObj != null) {
	                            try { code = Integer.parseInt(codeObj.toString()); } catch (Exception ignore) {}
	                        }
	                        ...
	                        // ✅ Lee idTipoUso del item
	                        if (item.get("idTipoUso") instanceof Number n) {
	                            jsonIns.put("idTipoUso", n.intValue());
	                        }
	                        ...
	                        MapSqlParameterSource paramsIns = new MapSqlParameterSource()
	                                .addValue("jsonData", jsonbParamIns);
	                        ...
	                        log.error("SP retornó error item[{}] statusCode={} message={} error={}",
	                                i, statusGet, msg, mapGet.get("error"));
	                        allOk = false;
	                        resultados.add(Map.of(
	                                "index",   i,
	                                "status",  statusGet,
	                                "message", msg,
	                                "detalle", String.valueOf(mapGet.getOrDefault("error", "sin detalle"))));
	                    }
	            ...
	            ResponseDTO dto = ResponseDTO.builder()
	                    ...
	                    .code(http.value())
	                    .response(Map.of("resultados", resultados))
	                    .build();
	        ...
	        log.info("roGet single type={} value={}",
	                roGet != null ? roGet.getClass().getName() : "null", roGet);
	        ...
	        if (statusGet == 200) {
	            ...
	            if (payload.get("idTipoUso") instanceof Number n) {
	                jsonUpd.put("idTipoUso", n.intValue());
	            }
	            ...
	            jsonUpd.put("usuarioModificacion", payload.getOrDefault("usuarioModificacion",
	                    payload.getOrDefault("usuarioCreacion", "system")));
	            ...
	            HttpStatus status = HttpStatus.resolve(code);
	            if (status == null) status = HttpStatus.CREATED;
	            ...
	            String msg = String.valueOf(mapGet.getOrDefault("message", "Fallo consultando existencia"));
	            log.error("SP retornó error single statusCode={} message={} error={}",
	                    statusGet, msg, mapGet.get("error"));
	            ResponseDTO dto = ResponseDTO.builder()
	                    .success(false)
	                    .message(msg)
	                    .code(statusGet)
