...
public final class LecturaSpecifications {
	...
	public static Specification<LecturaEntity> perteneceAEmpresa(Integer empresaId) {
        ...
        return (root, query, cb) -> {
            ...
            sub.select(ecc.get("contador").get("id"))
               .where(
    ...
    public static Specification<LecturaEntity> fechaBetween(java.time.LocalDate desde,
                                                            java.time.LocalDate hasta) {
        if (desde == null && hasta == null) return null;
        return (root, query, cb) -> {
            if (desde != null && hasta != null) {
                var iDesde = desde.atStartOfDay(java.time.ZoneId.systemDefault()).toInstant();
                var iHasta = hasta.atTime(java.time.LocalTime.MAX)
                                  .atZone(java.time.ZoneId.systemDefault()).toInstant();
                return cb.between(root.get("fechaLectura"),
                        java.util.Date.from(iDesde), java.util.Date.from(iHasta));
            } else if (desde != null) {
                var iDesde = desde.atStartOfDay(java.time.ZoneId.systemDefault()).toInstant();
    ...
    public static Specification<LecturaEntity> consumoAnormalEquals(Boolean consumoAnormal) {
        if (consumoAnormal == null) return null;
    ...
    public static Specification<LecturaEntity> observacionLike(String observacion) {
        ...
        return (root, query, cb) ->
            cb.like(cb.upper(root.get("descripcion")), "%" + observacion.trim().toUpperCase() + "%");
    }
}
