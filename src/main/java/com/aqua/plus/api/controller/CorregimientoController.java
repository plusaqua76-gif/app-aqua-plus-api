...
public class CorregimientoController {
    ...
    @Operation(summary = "Buscar corregimiento por id de ciudad")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "201", description = "Se ha guardado satisfactoriamente", content = {
                    @Content(mediaType = "application/json", schema = @Schema(implementation = ResponseDTO.class)) }),
            @ApiResponse(responseCode = "400", description = "La petición no puede ser entendida por el servidor debido a errores de sintaxis", content = {
    ...
    public ResponseEntity<ResponseDTO> getByCiudadId(@PathVariable Integer id) {
        return corregimientoServiceImpl.findByCiudadId(id);
    }
