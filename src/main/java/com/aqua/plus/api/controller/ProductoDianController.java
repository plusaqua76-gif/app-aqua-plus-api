...
public class ProductoDianController {
    ...
    public ResponseEntity<ResponseDTO> consultarProductos() {
        return productoDianService.consultarProductos();
    }
	
	@Operation(summary = "Buscar producto por codigo de unidad")
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Se consulta por codigo de unidad", content = {
					@Content(mediaType = "application/json", schema = @Schema(implementation = ResponseDTO.class)) }),
			@ApiResponse(responseCode = "400", description = "La petición no puede ser entendida por el servidor debido a errores de sintaxis", content = {
					@Content(mediaType = "application/json", schema = @Schema(implementation = ResponseDTO.class)) }),
			@ApiResponse(responseCode = "404", description = "El recurso solicitado no puede ser encontrado", content = {
			...
			@ApiResponse(responseCode = "500", description = "Se presentó una condición inesperada que impidió completar la petición", content = {
					@Content(mediaType = "application/json", schema = @Schema(implementation = ResponseDTO.class)) }), })
	@GetMapping("/{codigoUnidad}")
	public ResponseEntity<ResponseDTO> consultarProductoPorCodigo(@PathVariable(name = "codigoUnidad") String codigoUnidad) {
		return this.productoDianService.consultarProductoPorCodigo(codigoUnidad);
	}
}
